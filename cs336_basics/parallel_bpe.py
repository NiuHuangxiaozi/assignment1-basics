...
def train_bpe(
    ...
    **kwargs    
    ...
    with open(input_path, "r") as f:
        corpus = f.read()
    subword2count : dict[tuple[bytes], int]  = parallel_pre_tokenize(corpus, special_tokens, pat=PAT, n_workers=2)
...
def chunkify_corpus(corpus: str, special_tokens: List[str], n_workers: int ) -> Tuple[List[str], int]:
    '''
        这个函数标清除划分的边界，左闭右开
    '''
    token = special_tokens[0]
    positions = [0]
    start = 0
    while True:
        ...
        start = idx + len(token)
    positions.append(len(corpus))
    
    
    # group 操作
    if len(positions)-1 < n_workers:
        actual_workers = len(positions)-1
        new_positions = positions
    else:
        actual_workers = n_workers
        # 需要进行group
        group_size = (len(positions)-1) // n_workers
        new_positions = [positions[i] if i != 0 else 0 for i in range(0, len(positions), group_size)]
    ...
    for i in range(len(new_positions)-1):
        seg = corpus[new_positions[i]: new_positions[i+1]]
        segments.append(seg)
    return segments, actual_workers




def process_chunk(chunk: str, pat: str, special_tokens:List[str]) -> Dict[tuple[bytes], int]:
    '''
        每一个进程处理一小段文本的数据，转化为字典
    '''
    # 按照<|endoftext|> 进行切分
    splited_text = split_on_special_tokens(text=chunk, special_tokens = special_tokens)
    local: Dict[tuple[bytes], int] = {}
    for text in splited_text:
        for tok in regex.finditer(pat, text):
            s = tok.group(0)
            b = s.encode("utf-8")
            tup = tuple([b[i:i+1] for i in range(len(b))])
            local[tup] = local.get(tup, 0) + 1
    return local
...
def process_segment_wrapper(args):
    seg, pat, special_tokens = args
    return process_chunk(seg, pat, special_tokens)

def parallel_pre_tokenize(corpus: str, special_tokens: List[str], pat: str, n_workers: int) -> Dict[tuple[bytes], int]:
    '''
        多进程分发函数
    '''
    segments, new_numworker = chunkify_corpus(corpus, special_tokens,n_workers)
    args_list = [(seg, pat, special_tokens) for seg in segments]
    
    # Optional: if segments more than workers, you can group some segments together
    with multiprocessing.Pool(new_numworker) as pool:
        results = pool.map(process_segment_wrapper, args_list)
